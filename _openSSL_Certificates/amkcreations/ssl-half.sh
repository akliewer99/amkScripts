...
myDaysTL="3650"
myCommonName="egw.amkcreations.com"
...
if [ $# -eq 0 ] ; then
  echo -e '\n SYNTAX: sudo ./ssl-half.sh DomainName SerialNum CAName ready\n' 
	echo -e '  EXAMPLE:'
...
# DON'T CHANGE BELOW ITEMS UNLESS YOUR CAREFUL
fname="$1"
sn="$2"
myCompany="$3"
myCommonName="$fname"
...
workingdir=`pwd`
#
CAfnameKEY="$3.CA.PrivKey.pem"
CAfnameCRT="$3.CA.CRT.pem"
#
...
#openssl genrsa -des3 -out $CAfnameKEY -passout pass:"$myPassPhrase" 4096
#echo -e "\nCA Private key completed\n"

#echo -e "\nNow generating the CA Certificate file\n"
#openssl req -new -x509 -days 3650 -key $CAfnameKEY -out $CAfnameCRT -config openssl.cnf -batch -passin pass:"$myPassPhrase" -passout pass:"$myPassPhrase"
...
openssl x509 -req -days $myDaysTL -in $fnameServerCSR -CA $CAfnameCRT -CAkey $CAfnameKEY -set_serial $sn -out $fnameServerCRT -passin pass:"$myPassPhrase"
echo -e "\nSigning completed\n"
...
#****** testing *******
echo -e "\nCopy FILE:$fnameServerKEY to /etc/apache2/certificates\n"
/usr/bin/expect <<EOD
spawn sudo cp $fnameServerKEY /etc/apache2/certificates/$fnameServerKEY
expect -re "Password:"
send "$mySudoPwd"
EOD
echo -e "\nFile Copy SUCCESFUL\n"
...
/usr/bin/expect <<EOD
spawn sudo cp $fnameServerCRT /etc/apache2/certificates/$fnameServerCRT
expect -re "Password:"
send "$mySudoPwd"
EOD
echo -e "\nFile Copy SUCCESFUL\n"
...
/usr/bin/expect <<EOD
spawn sudo chmod 600 /etc/apache2/certificates/$fnameServerCRT
expect -re "Password:"
send "$mySudoPwd"
EOD
