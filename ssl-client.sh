...
if [ $# -le 4 ] ; then
  ...
  exit ;
		...
		#
		# USER VARIABLES
		fnameUserKEYinsecure="$domain.user.key.insecure.pem"
		fnameUserKEY="$domain.user.key.pem"
		fnameUserCERT="$domain.user.cert.pem"
		#
		fnameUserCSR="$domain.user.csr.pem"
		fnameServerCRL="$domain.server.crl.pem"
		#
		...
		doThis=`openssl ca -config $configFile \
						-extensions server_cert -days $myDaysTL -md sha256 \
						-in $csr/$fnameServerCSR -passin pass:$myPassPhrase \
						-passout pass:$myPassPhrase -out $certs/$fnameServerCERT`						
		amkDoCommand $stepName $doThis
		# ----------------------------------------------------------------------
		#
